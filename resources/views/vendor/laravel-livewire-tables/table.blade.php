        ...
        @if($show_search ?? true )
            <div class="w-full">
                <div class="flex items-center w-10/12 p-2 bg-white border border-gray-300 rounded md:w-5/12">
                    ...
                    <input type="search" class="w-full h-6 px-3 border-0 focus:outline-none" placeholder="{{ __('Search') }}" wire:model="search">
                </div>
            </div>
        @endif
        @if($header_view)
                    ...
                    <table class="table {{ $table_class }} mb-0">
                        ...
                        <tr>
                            ...
                            @foreach($columns as $column)
                                <th class="ltr:text-left rtl:text-right align-middle text-nowrap border-top-0 {{ $this->thClass($column->attribute) }}">
                                    @if($column->sortable)
